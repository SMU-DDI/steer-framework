...
Additional Note for Windows users: The windows newlines in `build.sh`, the bash-utilities directory, and the build_files directory may be hidden. To check, you can run `grep -n $'\r' /src/steer-framework/<INSERT_PATH_TO_FILE> | cat -v` which will search the file for hidden windows newlines and print the line along with the line number of the file. If you see `^M`, that is a windows newline which needs to be removed. To remove the windows newlines, you can run `sed -i 's/\r$//' /src/steer-framework/<INSERT_PATH_TO_FILE>`. This command combs through a file and removes trailing windows newlines.
