...
//! @brief This file implements the multi-threaded version of the NIST STS granger test 
...
typedef struct tnist_grangertestdata
{
...
tNIST_GrangerTestData;

typedef struct tnist_grangerthread
{
    pthread_t                           threadId;
    uint64_t                            testId;
    tNIST_GrangerTestData    testData;
}
tNIST_GrangerThread;
                      ...
                      tNIST_GrangerTestData* testData)
{
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        memset((void*)calculationStr, 0, STEER_STRING_MAX_LENGTH);
        sprintf(calculationStr, STEER_DEFAULT_FLOATING_POINT_STRING_FORMAT, 
                STEER_DEFAULT_FLOATING_POINT_PRECISION, testData->granger);
        result = STEER_AddCalculationToTest(report, 0, testId,
...
// =================================================================================================
int32_t GetNextAvailableThread (tNIST_GrangerPrivateData* privateData)
{
    int32_t index = -1;
    uint_fast32_t i = 0;
...
//  ThreadsReady
// =================================================================================================
bool ThreadsReady (tNIST_GrangerPrivateData* privateData)
{
    bool ready = true;
    uint_fast32_t i = 0;
...
{
    tNIST_GrangerTestData* testData = (tNIST_GrangerTestData*)ptr;
	int_fast32_t i = 0;
    int_fast32_t j = 0;
    int32_t k = 0;
    int32_t r = 0;
    ...
    if (testData->result == STEER_RESULT_SUCCESS)
    {
        testData->granger = entropyDistribution[0] - entropyDistribution[1];
        testData->chiSquared = 2.0 * seqLength * (gCommonData.log2pt0 - testData->granger);
        testData->probabilityValue = 
                  ...
                  uint64_t* bufferSizeInBytes)
{
    ...
    *bufferSizeInBytes = 0;
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        result = STEER_AllocateMemory(sizeof(tNIST_GrangerPrivateData) +
                                      (threadCount * sizeof(tNIST_GrangerThread)),
                                      (void**)&privData);
                     ...
                     uint64_t numOnes)
{
    ...
    {
        // Queue it up
        memset((void*)&(privData->thread[threadIndex].testData), 0, sizeof(tNIST_GrangerTestData));
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tNIST_GrangerPrivateData* privData = (tNIST_GrangerPrivateData*)(*privateData);
