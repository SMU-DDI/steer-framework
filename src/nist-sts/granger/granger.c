                ...
                bool *passed)
{
    ...
    *passed = false;
    ...
    result = STEER_AllocateMemory(6 * sizeof(char *), (void **)&args);
    if (result == STEER_RESULT_SUCCESS)
    {
        for (i = 0; i < 6; i++)
            result = STEER_AllocateMemory(sizeof(char *), (void **)&args[i]);
    }
    if (result == STEER_RESULT_SUCCESS)
            ...
            if (result == STEER_RESULT_SUCCESS)
            {
                for (i = 0; i < 3; i++)
                {
                    ...
                    if (i == 1)
                    {
                        ...
                        STEER_AllocateMemory(intStringLen, (void **)&intString);
    ...
    if (result == STEER_RESULT_SUCCESS)
        STEER_RunPython(GRANGER_PY_LOCATION, (const char **)args, 6, &pythonBuf);
