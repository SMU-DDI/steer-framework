...
//! @brief This file implements the NIST STS frequency test for the STEER framework.
...
#define PROGRAM_NAME        "nist_sts_frequency_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "frequency (monobit)"
...
The test assesses the closeness of the fraction of ones to ½, that is, the number of \
ones and zeroes in a sequence should be about the same. All subsequent tests depend \
...
typedef struct tnist_frequencyprivatedata
{
    ...
    uint64_t                    ones;
    uint64_t                    zeros;
    int32_t                     nthPartialSum;
    double                      nthPartialSumOverBitstreamLength;
    double                      probabilityValue;
}
tNIST_FrequencyPrivateData;
...
static tSTEER_InfoList gReferences = {
    ...
    { 
        ...
        "NIST Special Publication 800-22 Rev. 1a, Section 2.1",
        "NIST Special Publication 800-22 Rev. 1a, Section 2.1.7",
        "NIST Special Publication 800-22 Rev. 1a, Section 3.1",
...
static tSTEER_ParameterInfoList gParameterInfoList = {
    3,
    {
        ...
        // Required parameter
        {
            ...
            "1000000",
            "100",      // Verified (NIST SP 800-22 Rev 1a, 2.1.7)
            NULL
        },
...
//  RunTest
// =================================================================================================
int32_t RunTest (tNIST_FrequencyPrivateData* privateData,
                 ...
                 bool* passed)
{
    ...
    for (i = 0; i < privateData->bitstreamLength; i++)
        sum += (2 * (int32_t)bitstreamBuffer[i]) - 1;
    s_obs = fabs(sum)/sqrt(privateData->bitstreamLength);
    f = s_obs/sqrt2;
    privateData->probabilityValue = erfc(f);

    privateData->nthPartialSum = (int32_t)sum;
    privateData->nthPartialSumOverBitstreamLength = sum/privateData->bitstreamLength;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tNIST_FrequencyPrivateData* privData = NULL;
    ...
    *bufferSizeInBytes = 0;
    ...
    result = STEER_AllocateMemory(sizeof(tNIST_FrequencyPrivateData),
                                  (void**)&privData);
...
int32_t SetReport (void* testPrivateData,
                   tSTEER_ReportPtr report)
{
    ((tNIST_FrequencyPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tNIST_FrequencyPrivateData* privData = (tNIST_FrequencyPrivateData*)testPrivateData;
    bool passed = false;
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        // Nth partial sum
        memset((void*)calculationStr, 0, STEER_STRING_MAX_LENGTH);
        sprintf(calculationStr, STEER_DEFAULT_SIGNED_INTEGER_STRING_FORMAT, privData->nthPartialSum);
        result = STEER_AddCalculationToTest(privData->report, 0, testId,
                                            STEER_JSON_TAG_NTH_PARTIAL_SUM,
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        memset((void*)calculationStr, 0, STEER_STRING_MAX_LENGTH);
        sprintf(calculationStr, STEER_DEFAULT_FLOATING_POINT_STRING_FORMAT, 
                STEER_DEFAULT_FLOATING_POINT_PRECISION, privData->nthPartialSumOverBitstreamLength);
        result = STEER_AddCalculationToTest(privData->report, 0, testId,
                                            STEER_JSON_TAG_NTH_PARTIAL_SUM_OVER_BITSTREAM_LENGTH,
                                            STEER_JSON_VALUE_DOUBLE_PRECISION_FLOATING_POINT,
                                            STEER_JSON_VALUE_DEFAULT_FLOATING_POINT_PRECISION, 
                                            NULL, calculationStr);
    }
